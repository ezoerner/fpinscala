...
*/
  ...
  def traverse[A,B](la: List[A])(f: A => M[B]): M[List[B]] =
    la.foldLeft[M[List[B]]](unit(Nil))((acc, m) => map2(f(m),acc)(_ :: _))

  def replicateM[A](n: Int, ma: M[A]): M[List[A]] =
    sequence(List.fill(n)(ma))

  def replicateMRecursive[A](n: Int, ma: M[A]): M[List[A]] =
    if
      (n <= 0) unit(Nil)
    else
      map2(ma, replicateMRecursive(n-1, ma))(_ :: _)
  ...
  // compose in terms of join and map
...
object Monad {
  ...
  val optionMonad: Monad[Option] = new Monad[Option] {
    ...
    override def flatMap[A, B](ma: Option[A])
      (f: (A) ⇒ Option[B]): Option[B] = ma flatMap f
  }
  ...
  val idMonad: Monad[Id] = new Monad[Id] {
    ...
    override def flatMap[A, B](ma: Id[A])
      (f: (A) ⇒ Id[B]): Id[B] = ma flatMap f
  }
